...
In this version, Sandy Clay Loam was used (instead of coarse sand before)
First, a deep groundwater table was assumed, Z=1000 cm.

Experiments :

# Experiment 1: Salt on growth
-for climates 1-4
-with deep gw, Z=1000 cm
-with high gw, Z =100 cm  # NOT YET
...
##Experiment 1: Salt on Growth, not on mortality

# Salt influence on Plant growth function only, not on mortality
...
## Plant mortality function WITHOUT SALT INFLUENCE, NO VIRTUAL SATURATION
Plant mortality is just reduced by the linear constant d
```{r PLANT Mortality}
Mo <- function(P,M,par) {
...
# water uptake by plants: include infiltration in available water
      ...
      # Mortality
      Mo_sub[tt]<- Mo(P.old, M=M.old,par)*timeincr
...
# Histograms for experiment 1
```{r}
h1P<- summary(lambda_sum$P)
h1M <-summary(lambda_sum$M)
h1SmM<-summary(lambda_sum$SmM)
h1CM<-summary(lambda_sum$CM)
...
# water uptake by plants: include infiltration in available water
      ...
      WU_sub[tt] <- WU(M=M.old,P.old,par)*timeincr 
...
if (plotit==T) {  
  plot(M, type="l",ylim=c(0,100),xlim=c(0,time),xlab=("time [d]"), main=paste(" Exp 2  lambda=", lambda[j],"alpha=", alpha[i]))
  points(Rain*10, type="h", col="skyblue")
...
# Histograms for experiment 2
```{r}

h2P<- summary(lambda_sum$P)
h2M <-summary(lambda_sum$M)
h2SmM<-summary(lambda_sum$SmM)
h2CM<-summary(lambda_sum$CM)
...
```{r}
par(mfrow = c(1,2))
hist(h1P, breaks=10, freq=T, col='darkgreen', main='Comparing influence of salt on plant growth (green) vs mortality (black)', xlab='Plant biomass') 
hist(h2P, breaks=100, freq=T, col='black', add=T)

hist(h1M, breaks=10, freq=T, col='darkgreen', main='Comparing influence of salt on plant growth (green) vs mortality(black)', xlab='soil moisture') 
hist(h2M, breaks=10, freq=T, col='black', add=T)

hist(h1SmM, breaks=10, freq=T, col='darkgreen', main='Comparing influence of salt on plant growth (green) vs mortality(black)', xlab='soil salt mass') 
hist(h2SmM, breaks=10, freq=T, col='black', add=T)

hist(h1CM, breaks=10, freq=T, col='darkgreen', main='Comparing influence of salt on plant growth (green) vs mortality(black)', xlab='soil salt mass') 
hist(h2CM, breaks=1000, freq=T, col='black', add=T)
