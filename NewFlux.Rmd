...
#     nvg <- 1.237
  ...
  Zmax =1000 # cm GROUNDWATER DEPTH
  Z =100
...
# s=M/(soilpar$n*Zr)
# plot(s,L_n(M,Z=Z,Zmax=Zmax,soilpar=soilpar,vegpar=vegpar),type="l" )#,ylim=c(0,10)) xlim=c(0,1), ylim=c(0,20)
...
flux_sub[tt]<-do.call(L_n,list(M=M_sub[tt+1],Z=Z, Zmax=Zmax,soilpar=soilpar,vegpar=vegpar))
...
# calculate saltbalance
    ...
    L_salt <- ifelse(flux_sub[tt]<0, par$f*CM_sub[tt+1]*flux_sub[tt+1]*timeincr,0) # leaching of salt
    
    U_salt <- ifelse(flux_sub[tt]>0, par$CM.gw*flux_sub[tt+1]*timeincr,0) # rise of salt
...
summary(lambda_sum$SmM)
summary(lambda_sum$CM)
summary(lambda_sum$flux)
