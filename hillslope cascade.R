...
balances2D <- function(Rain, par=par_in,plotit=F,
  ...
  L<- matrix(0,nrow= length(Rain), ncol =grids)
  U<- matrix(0,nrow= length(Rain), ncol =grids)
  ...
  L_sub<- matrix(0,nrow= deltat, ncol =grids)  # leaching
  U_sub<-matrix(0,nrow= deltat, ncol =grids)  # capillary upflow
