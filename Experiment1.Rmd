...
#  Z <- 1000
# plot(M,L_n(M,Z,P=10,soilpar=soilpar,vegpar=vegpar)$L,type="l",ylim=c(0,10))
# lines(M,L_n(M,Z,P=10,soilpar=soilpar,vegpar=vegpar)$U,col="red")
```
...
# if(Overflow[tt] >0){
#       h_sub[tt+1] <- h.old + ifelse(tt==1,Rain[t],0) - Infil(h.old, P.old,par)*timeincr + Overflow[tt]
# }
# else{
...
#  h_sub[tt+1] <- h.old + ifelse(tt==1,Rain[t],0) - Infil(h.old, P.old,par)*timeincr
h_sub[tt+1] <- ifelse(Overflow[tt] >0,h.old + ifelse(tt==1,Rain[t],0) - Infil(h.old, P.old,par)*timeincr + Overflow[tt],h.old + ifelse(tt==1,Rain[t],0) - Infil(h.old, P.old,par)*timeincr)
...
# Calculate salt concentration in the soil
      CM_sub[tt+1]<- SmM_sub[tt+1]/M_sub[tt+1] #tt+1

      # 3. calculate leaching amount

   L_sub[tt]<-do.call(L_n,list(M = M_sub[tt + 1], P = P_sub[tt + 1], Z = Z,soilpar = soilpar,vegpar = vegpar))$L 
   
   U_sub[tt]<-do.call(L_n,list(M = M_sub[tt + 1],P = P_sub[tt + 1], Z = Z,soilpar = soilpar, vegpar = vegpar))$U

## If U exceeds what fits into M, it adds to h
  Overflow[tt] <- U_sub[tt]*timeincr-((soilpar$s_fc*(soilpar$n*vegpar$Zr))-M_sub[tt+1])

      # 4. final adjust soil moisture for leaching
# overflow onlz occurs when positive of course
# if(Overflow[tt] >0){
#        M_sub[tt + 1] <- M_sub[tt + 1] - L_sub[tt]*timeincr + U_sub[tt]*timeincr - Overflow[tt]
#        }
#   else 
#   {
...
#   }
  ...
  M_sub[tt + 1] <- ifelse(Overflow[tt] >0,M_sub[tt + 1] - L_sub[tt]*timeincr + U_sub[tt]*timeincr - Overflow[tt], M_sub[tt + 1] - L_sub[tt]*timeincr + U_sub[tt]*timeincr)

# calculate saltbalance
    L_salt <- CM.old*L_sub[tt]*timeincr
    U_salt <- par$CM.gw*U_sub[tt]*timeincr
       
      SmI_sub[tt+1]<- SmI.old + I_sub[tt]*par$ConcConst - L_salt
      SmM_sub[tt+1]<- SmM_sub[tt]+ SmI_sub[tt+1] + U_salt ### think about this again 
