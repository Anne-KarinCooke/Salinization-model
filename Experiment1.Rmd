...
# Balance for water depth on soil
      ...
      h_sub[tt+1] <- h.old + ifelse(tt==1,Rain[t],0) - Infil(h.old, P.old)*timeincr + Overflow
...
#    L_sub[tt] <- L_n(M=M_sub[tt],Z=Z,soilpar=soilpar,vegpar=vegpar)$L
   ...
   # U_sub[tt]<-do.call(L_n,list(M=M_sub[tt + 1],Z=Z,soilpar=soilpar,vegpar=vegpar))$U
      # 4. final adjust soil moisture for leaching
       M_sub[tt + 1] <- M_sub[tt + 1] - L_sub[tt]*timeincr + U_sub[tt]*timeincr - Overflow

  if (U_sub[tt]> soilpar$s_fc-(M_sub[tt]/(soilpar$n*soilpar$Zr))){
     Overflow <- U_sub[tt]-(soilpar$s_fc-(M_sub[tt]/(soilpar$n*soilpar$Zr))
  }
...
# calculate saltbalance
    ...
    U_salt <- CM.gw*U_sub[tt]*timeincr
      ...
      SmM_sub[tt+1]<- SmI_sub[tt + 1] + U_salt
...
for (j in 1:length(lambda)) {
    ...
    sub_store[[j]] <-data.frame(alpha_o=rep(alpha[i],time),
                                ...
                                balances(Rain,plotit=T, par=par_in,
