...
# We decided to split the numerical calculations for the daily into 12 substeps.
   ...
   deltat <- 100 # split in 12 increments
...
#   }
  ...
  M_sub[tt + 1] <- M_sub[tt + 1] - L_sub[tt]*timeincr + U_sub[tt]*timeincr 
    #ifelse(Overflow[tt+1] >0,M_sub[tt + 1] - L_sub[tt]*timeincr + U_sub[tt]*timeincr - Overflow[tt], M_sub[tt + 1] - L_sub[tt]*timeincr + U_sub[tt]*timeincr)
#  U_sub[tt] <- ifelse(Overflow[tt+1] >0,U_sub[tt] - Overflow[tt+1]/timeincr, U_sub[tt]) 
...
for (j in 1:length(lambda)) {
    ...
    sub_store[[j]] <-data.frame(alpha_o=rep(alpha[i],time),
                                ...
                                balances(Rain[1:10],plotit=F, par=par,
                                soilpar, vegpar))
  }
  Store[[i]] <- sub_store
}
