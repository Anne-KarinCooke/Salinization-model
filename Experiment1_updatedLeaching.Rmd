...
L_n <- function(M,Z,WU,soilpar,vegpar) {
  ...
  u <- c(rep(G1*soilpar$K_s*(1-exp(-(vegpar$c_p*WU)))*soilpar$Ep,length(u_set)),rep(0,(length(M)-length(u_set))))
...
# calculate saltbalance
    ...
    U_salt <- par$CM.gw*U_sub[tt]*timeincr
      ...
      SmM_sub[tt+1]<- SmI_sub[tt+1] + U_salt - L_salt ### think about this again 
